export const OPENAPI_DOCUMENT = Object.freeze({
  ...
  paths: {
    ...
    "/v1/public/deletion-requests": { post: { security: [], description: "Accept a non-enumerating public deletion request and send a possession link when configured", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/PublicDeletionRequest" } } } }, responses: { "202": { description: "Non-enumerating request accepted" }, "400": { description: "Invalid request" }, "503": { description: "Deletion email delivery unavailable" } } } },
    "/v1/public/deletion-requests/{requestId}/confirm": { post: { security: [], description: "Verify possession with the expiring deletion token and execute deletion idempotently", parameters: [{ name: "requestId", in: "path", required: true, schema: { type: "string", format: "uuid" } }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/PublicDeletionConfirm" } } } }, responses: { "200": { description: "Deletion proof complete" }, "400": { description: "Invalid or expired possession token" }, "503": { description: "Remote deletion pending" } } } },
    ...
    "/v1/public/deletion-operations/status": { post: { security: [], description: "Resume or read deletion progress only with the operation id and the local possession-bound uid hash", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/PublicDeletionStatus" } } } }, responses: { "200": { description: "Deletion operation status" }, "404": { description: "Operation unavailable" } } } },
  ...
  components: {
    ...
    schemas: {
      ...
      PublicDeletionRequest: { type: "object", additionalProperties: false, required: ["email"], properties: { email: { type: "string", format: "email", maxLength: 320 } } },
      PublicDeletionConfirm: { type: "object", additionalProperties: false, required: ["token"], properties: { token: { type: "string", minLength: 20, maxLength: 128 } } },
      PublicDeletionStatus: { type: "object", additionalProperties: false, required: ["operationId", "accountUidHash"], properties: { operationId: { type: "string", format: "uuid" }, accountUidHash: { type: "string", pattern: "^[a-f0-9]{64}$" } } },
      CreateContentReport: {
        type: "object",
