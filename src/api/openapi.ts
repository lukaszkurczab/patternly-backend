...
const adoptionTransferRecordOpenApi = {
  type: "object",
  additionalProperties: false,
  required: ["recordType", "recordId", "trackId", "fingerprint", "state", "version"],
  properties: adoptionTransferRecordBaseProperties,
};
const adoptionTransferStartOpenApi = {
  type: "object",
  additionalProperties: false,
  required: ["canonicalVersion", "idempotencyKey", "guestUserId", "snapshotVersion", "deviceId"],
  properties: {
...
const adoptionTransferUploadOpenApi = {
  type: "object",
  additionalProperties: false,
  required: ["canonicalVersion", "deviceId", "chunk", "records"],
  properties: {
    canonicalVersion: { type: "string", const: "canonical-json-v1", default: "canonical-json-v1" },
    idempotencyKey: { type: "string", minLength: 1, maxLength: 128 },
    deviceId: { type: "string", format: "uuid" },
...
const adoptionTransferSealOpenApi = {
  type: "object",
  additionalProperties: false,
  required: ["canonicalVersion", "deviceId", "snapshotFingerprint", "recordCount", "chunkCount"],
  properties: {
    canonicalVersion: { type: "string", const: "canonical-json-v1", default: "canonical-json-v1" },
    idempotencyKey: { type: "string", minLength: 1, maxLength: 128 },
    deviceId: { type: "string", format: "uuid" },
...
const adoptionTransferPreviewOpenApi = {
  type: "object",
  additionalProperties: false,
  required: ["canonicalVersion", "deviceId"],
  properties: {
    canonicalVersion: { type: "string", const: "canonical-json-v1", default: "canonical-json-v1" },
    idempotencyKey: { type: "string", minLength: 1, maxLength: 128 },
    deviceId: { type: "string", format: "uuid" },
...
const adoptionTransferConfirmOpenApi = {
  type: "object",
  additionalProperties: false,
  required: ["canonicalVersion", "deviceId", "previewFingerprint", "resolutions", "groupChoices"],
  properties: {
    canonicalVersion: { type: "string", const: "canonical-json-v1", default: "canonical-json-v1" },
    idempotencyKey: { type: "string", minLength: 1, maxLength: 128 },
    deviceId: { type: "string", format: "uuid" },
...
const adoptionTransferApplyOpenApi = {
  type: "object",
  additionalProperties: false,
  required: ["canonicalVersion", "deviceId"],
  properties: {
...
const adoptionTransferStatusOpenApi = { type: "object", additionalProperties: false, required: ["accountId", "sessionId", "guestUserId", "state", "expectedGeneration", "generation", "targetGeneration", "snapshotVersion", "recordCount", "chunkCount", "snapshotFingerprint", "previewFingerprint", "operationId", "decisionFingerprint", "applyCursor", "applyTotal", "accountRevision", "failureCode"], properties: adoptionTransferStatusBaseProperties };
...
const OPENAPI_DOCUMENT_RAW = {
  ...
  paths: {
    ...
    "/v1/account/registration": { "x-patternly-security-profile": "app_check_verify_only_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Atomically register the verified identity and immutable Terms acceptance plus Privacy Policy acknowledgement. Replays for an existing account never alter legal evidence.", responses: { "200": { description: "Identity already has a Patternly account; no legal evidence changed" }, "201": { description: "Account and registration evidence created atomically" }, "400": { description: "Invalid registration evidence" }, "401": { description: "Authentication failed or an active deletion tombstone blocks registration" } } } },
    "/v1/account/session/exchange": { "x-patternly-security-profile": "app_check_verify_only_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Exchange a recently authenticated Firebase identity for a custom token pinned to the active account's authorization generation.", requestBody: { required: false, content: { "application/json": { schema: { "$ref": "#/components/schemas/EmptyRequest" } } } }, responses: { "200": { description: "Generation-pinned Firebase custom token issued" }, "400": { description: "Invalid request" }, "401": { description: "Account is deleted or Firebase authentication is not newer than its rotation barrier" }, "404": { description: "Verified identity has no Patternly account" } } } },
    "/v1/me": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", get: { responses: { "200": { description: "Canonical account identity" }, "401": { description: "Authentication required, account deleted, or authorization generation claim missing, malformed, or stale", "x-patternly-error-codes": ["authentication_required", "account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "404": { description: "Verified identity has no Patternly account" } } } },
    "/v1/legal-acceptances": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Record an immutable, versioned Terms and minimum-age acceptance for the authenticated account.", responses: { "201": { description: "Acceptance recorded" }, "400": { description: "Invalid acceptance" }, "401": { description: "Authentication or authorization generation claim is invalid", "x-patternly-error-codes": ["authentication_required", "account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "409": { description: "Account authorization generation changed after request authentication", "x-patternly-error-codes": ["authorization_generation_conflict"] } } } },
    "/v1/purchase-confirmations": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Record the immutable pre-contract offer and express immediate-start request before opening App Store checkout.", responses: { "201": { description: "Confirmation recorded" }, "400": { description: "Invalid confirmation" }, "401": { description: "Authentication or authorization generation claim is invalid", "x-patternly-error-codes": ["authentication_required", "account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "409": { description: "Legal acceptance, purchase attempt, or account authorization generation conflict", "x-patternly-error-codes": ["legal_acceptance_required", "purchase_attempt_active", "authorization_generation_conflict"] } } } },
    "/v1/entitlements": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", get: { responses: { "200": { description: "Fresh RevenueCat entitlement for the authenticated account" }, "503": { description: "RevenueCat read unavailable" } } } },
    "/v1/progress": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", get: { parameters: [{ name: "pageSize", in: "query", required: false, schema: { type: "integer", minimum: 1, maximum: 100 } }, { name: "pageToken", in: "query", required: false, schema: { type: "string", minLength: 1 } }], responses: { "200": { description: "Canonical progress state and account revision; paged responses include generation and nextPageToken" }, "400": { description: "Invalid pagination request or token" }, "409": { description: "Progress generation, account revision, or persisted content identity conflict" } } } },
    "/v1/account-data/export": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", get: { description: "Download a bounded JSON account-data export after recent reauthentication and a final authorization-generation check. The response is an attachment and is never persisted as a payload.", responses: { "200": { description: "JSON account-data attachment", headers: { "Cache-Control": { schema: { type: "string", const: "private, no-store" } }, "Content-Disposition": { schema: { type: "string" } } }, content: { "application/json": { schema: { "$ref": "#/components/schemas/AccountDataExport" } } } }, "401": { description: "Authentication, recent reauthentication, or active account required", "x-patternly-error-codes": ["account_deleted", "authorization_generation_required", "authorization_generation_invalid", "recent_reauthentication_required"], headers: { "Cache-Control": { schema: { type: "string", const: "private, no-store" } } } }, "409": { description: "Account authorization generation changed before export completion", "x-patternly-error-codes": ["authorization_generation_conflict"], content: { "application/json": { schema: { "$ref": "#/components/schemas/ErrorEnvelope" } } } }, "413": { description: "Serialized export exceeds the configured maximum" }, "429": { description: "Per-account export rate limit exceeded", headers: { "Retry-After": { schema: { type: "integer", minimum: 1 } } } } } } },
    "/v1/privacy-requests": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Create one authenticated privacy-right case", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/CreateAccountPrivacyRequest" } } } }, responses: { "201": { description: "Privacy request created" }, "401": { description: "Authentication, recent reauthentication, or active account required", "x-patternly-error-codes": ["account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "409": { description: "Account authorization generation changed before the request was created", "x-patternly-error-codes": ["authorization_generation_conflict"] } } }, get: { description: "List minimal metadata for the authenticated subject's own privacy requests", responses: { "200": { description: "Privacy request list" } } } },
    "/v1/privacy-requests/{requestId}": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", get: { description: "Read one own privacy response after recent reauthentication", parameters: [{ "$ref": "#/components/parameters/PrivacyRequestId" }], responses: { "200": { description: "Privacy response" }, "401": { description: "Recent reauthentication or active account required", "x-patternly-error-codes": ["account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "404": { description: "Request unavailable" }, "409": { description: "Account authorization generation changed before privacy detail was returned", "x-patternly-error-codes": ["authorization_generation_conflict"] } } } },
    "/v1/guest/privacy-requests": { "x-patternly-security-profile": "app_check_only", "x-patternly-consumer-scope": "mobile", post: { description: "Record an idempotent guest privacy request. Email delivery is unconfirmed and sender outcomes are not disclosed.", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/CreateGuestPrivacyRequest" } } } }, responses: { "202": { description: "Request pending verification" }, "401": { description: "App Check required" }, "409": { description: "Idempotency key reused with different payload" }, "429": { description: "Rate limited" } } } },
    "/v1/guest/privacy-requests/verify": { "x-patternly-security-profile": "app_check_only", "x-patternly-consumer-scope": "mobile", post: { description: "Exchange a one-use email code for a short request-scoped session", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/GuestPrivacyCode" } } } }, responses: { "200": { description: "Short guest privacy session" }, "404": { description: "Uniform unavailable response" }, "429": { description: "Rate limited" } } } },
    "/v1/guest/privacy-requests/{requestId}/resend": { "x-patternly-security-profile": "app_check_only", "x-patternly-consumer-scope": "mobile", post: { description: "Non-enumerating request to send a fresh in-app email code; delivery outcome is not disclosed.", parameters: [{ "$ref": "#/components/parameters/PrivacyRequestId" }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/GuestPrivacyResend" } } } }, responses: { "202": { description: "Verification pending, regardless of whether the request ID or email matches" }, "429": { description: "Rate limited" } } } },
    "/v1/guest/privacy-requests/{requestId}/response": { "x-patternly-security-profile": "app_check_only", "x-patternly-consumer-scope": "mobile", post: { description: "Read a guest privacy response with a short request-scoped session", parameters: [{ "$ref": "#/components/parameters/PrivacyRequestId" }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/PublicPrivacySession" } } } }, responses: { "200": { description: "Privacy response" }, "404": { description: "Uniform unavailable response" } } } },
    "/v1/legal-requests": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Create an authenticated consumer complaint, withdrawal, non-personal data recovery request, or suspension appeal and email a durable receipt.", responses: { "201": { description: "Consumer case received" }, "401": { description: "Authentication required or account unavailable", "x-patternly-error-codes": ["authentication_required", "account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "409": { description: "Account authorization changed during case creation", "x-patternly-error-codes": ["authorization_generation_conflict"] }, "503": { description: "Email delivery unavailable" } } }, get: { description: "List the authenticated account's consumer cases.", responses: { "200": { description: "Consumer case list" } } } },
    "/v1/legal-requests/{requestId}": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", get: { description: "Read one consumer case owned by the authenticated account.", responses: { "200": { description: "Consumer case" }, "404": { description: "Case unavailable" } } } },
    "/v1/public/legal-requests": { "x-patternly-security-profile": "app_check_optional_bearer", "x-patternly-consumer-scope": "mobile", post: { security: [{ appCheckAuth: [] }], description: "Create a guest consumer case with App Check, bounded rate limiting, and a durable email receipt.", parameters: [{ "$ref": "#/components/parameters/FirebaseAppCheck" }], responses: { "201": { description: "Consumer case received" }, "401": { description: "Valid App Check required or authenticated account unavailable", "x-patternly-error-codes": ["app_check_required", "app_check_invalid", "authentication_required", "account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "409": { description: "Account authorization changed during case creation", "x-patternly-error-codes": ["authorization_generation_conflict"] }, "429": { description: "Rate limited" }, "503": { description: "Email delivery unavailable" } } } },
    "/v1/progress/sync": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/SyncRequest" } } } }, responses: { "200": { description: "Applied or duplicate mutations" }, "400": { description: "Invalid sync request, fingerprint, or track bundle" }, "401": { description: "Account is inactive", "x-patternly-error-codes": ["account_deleted"] }, "409": { description: "Progress, account revision, authorization generation, or adoption promotion conflict", "x-patternly-error-codes": ["authorization_generation_conflict", "progress_generation_conflict"] }, "413": { description: "Canonical UTF-8 sync envelope exceeds 512 KiB" } } } },
    "/v1/account-data/adoption/preview": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Preview local account-data adoption without binding or writing learning records", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/GuestMergeSnapshot" } } } }, responses: { "200": { description: "Deterministic adoption preview" }, "400": { description: "Invalid snapshot" } } } },
    "/v1/account-data/adoption/confirm": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Execute an exact confirmed adoption preview idempotently", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/AdoptionConfirmationRequest" } } } }, responses: { "200": { description: "Adoption executed or replayed idempotently" }, "401": { description: "Account is inactive", "x-patternly-error-codes": ["account_deleted"] }, "409": { description: "Preview, resolution, active-session, idempotency, or authorization generation conflict", "x-patternly-error-codes": ["authorization_generation_conflict"] } } } },
    "/v1/account-data/adoption/transfer/start": { "x-patternly-security-profile": "bearer", "x-patternly-consumer-scope": "backend-only", post: { description: "Start an authenticated, resumable account-data transfer session", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/AdoptionTransferStart" } } } }, responses: { "200": { description: "Transfer session created or replayed idempotently" }, "400": { description: "Invalid transfer start" }, "401": { description: "Authentication required or account inactive", "x-patternly-error-codes": ["account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "409": { description: "Authorization-generation, transfer-generation, or idempotency conflict" } } } },
    "/v1/account-data/adoption/transfer/{sessionId}/upload": { "x-patternly-security-profile": "bearer", "x-patternly-consumer-scope": "backend-only", post: { description: "Upload one bounded transfer chunk; records and chunk metadata are stored separately", parameters: [{ "$ref": "#/components/parameters/AdoptionTransferSessionId" }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/AdoptionTransferUpload" } } } }, responses: { "200": { description: "Chunk accepted or replayed" }, "400": { description: "Invalid record, chunk, or fingerprint" }, "401": { description: "Authentication rejected or account inactive", "x-patternly-error-codes": ["account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "404": { description: "Transfer unavailable" }, "409": { description: "Authorization-generation, duplicate, state, or idempotency conflict" }, "413": { description: "Canonical envelope or record limit exceeded" } } } },
    "/v1/account-data/adoption/transfer/{sessionId}/seal": { "x-patternly-security-profile": "bearer", "x-patternly-consumer-scope": "backend-only", post: { description: "Seal a complete account-data transfer by canonical digest", parameters: [{ "$ref": "#/components/parameters/AdoptionTransferSessionId" }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/AdoptionTransferSeal" } } } }, responses: { "200": { description: "Transfer sealed or replayed" }, "400": { description: "Invalid seal" }, "401": { description: "Authentication rejected or account inactive", "x-patternly-error-codes": ["account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "404": { description: "Transfer unavailable" }, "409": { description: "Authorization-generation, incomplete, stale, or mismatched seal conflict" } } } },
    "/v1/account-data/adoption/transfer/{sessionId}/preview": { "x-patternly-security-profile": "bearer", "x-patternly-consumer-scope": "backend-only", post: { description: "Build a resumable deterministic account-data preview using the merge semantics", parameters: [{ "$ref": "#/components/parameters/AdoptionTransferSessionId" }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/AdoptionTransferPreview" } } } }, responses: { "200": { description: "Preview ready" }, "400": { description: "Invalid or unsupported merge" }, "401": { description: "Account authorization is no longer active" }, "404": { description: "Transfer unavailable" }, "409": { description: "Preview or generation conflict" } } } },
    "/v1/account-data/adoption/transfer/{sessionId}/confirm": { "x-patternly-security-profile": "bearer", "x-patternly-consumer-scope": "backend-only", post: { description: "Persist an immutable transfer decision fingerprint and exact conflict resolutions in authorization-fenced chunks", parameters: [{ "$ref": "#/components/parameters/AdoptionTransferSessionId" }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/AdoptionTransferConfirm" } } } }, responses: { "200": { description: "Decision recorded or replayed" }, "400": { description: "Invalid confirmation" }, "401": { description: "Authentication rejected or account inactive", "x-patternly-error-codes": ["account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "404": { description: "Transfer unavailable" }, "409": { description: "Authorization-generation, preview, decision, device, or resolution conflict", "x-patternly-error-codes": ["authorization_generation_conflict"] } } } },
    "/v1/account-data/adoption/transfer/{sessionId}/apply": { "x-patternly-security-profile": "bearer", "x-patternly-consumer-scope": "backend-only", post: { description: "Materialize confirmed adoption in authorization-fenced transactions. Each target chunk and its cursor commit together; the active generation changes only after every chunk completes.", parameters: [{ "$ref": "#/components/parameters/AdoptionTransferSessionId" }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/AdoptionTransferApply" } } } }, responses: { "200": { description: "Applied or resumed atomically" }, "400": { description: "Invalid apply request" }, "401": { description: "Authentication rejected or account inactive", "x-patternly-error-codes": ["account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "404": { description: "Transfer unavailable" }, "409": { description: "Authorization, source generation, cursor, lease, or decision conflict", "x-patternly-error-codes": ["authorization_generation_conflict"] }, "413": { description: "A transfer record exceeds the safe transaction budget" } } } },
    "/v1/account-data/adoption/transfer/{sessionId}/status": { "x-patternly-security-profile": "bearer", "x-patternly-consumer-scope": "backend-only", get: { description: "Read bounded resumable account-data transfer status", parameters: [{ "$ref": "#/components/parameters/AdoptionTransferSessionId" }, { name: "deviceId", in: "query", required: true, schema: { type: "string", format: "uuid" } }], responses: { "200": { description: "Transfer status", content: { "application/json": { schema: { "$ref": "#/components/schemas/AdoptionTransferStatus" } } } }, "400": { description: "Invalid or mismatched device" }, "401": { description: "Authentication rejected or account inactive", "x-patternly-error-codes": ["account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "404": { description: "Transfer unavailable" } } } },
    "/v1/account/recovery-codes": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Replace the account's ten one-time recovery codes after recent reauthentication", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/EmptyRequest" } } } }, responses: { "200": { description: "Recovery codes returned once" }, "401": { description: "Recent reauthentication required or account is inactive", "x-patternly-error-codes": ["recent_reauthentication_required", "account_deleted", "authorization_generation_required", "authorization_generation_invalid"] }, "409": { description: "Authorization generation changed before recovery codes were issued", "x-patternly-error-codes": ["authorization_generation_conflict"] } } } },
    "/v1/public/recovery-codes/consume": { "x-patternly-security-profile": "app_check_only", "x-patternly-consumer-scope": "mobile", post: { security: [], description: "Consume one possession recovery code and issue a Firebase custom token", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/RecoveryCodeConsumeRequest" } } } }, responses: { "200": { description: "Recovery code consumed" }, "401": { description: "Invalid recovery code" }, "409": { description: "Recovery code already used" } } } },
    "/v1/account/session/revoke": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Revoke current and stale Firebase sessions through the active account security-operation slot, then issue a replacement custom token for the same Firebase subject and authorization generation. The operation ID is idempotent and bound to the presented authorization generation.", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/OperationRequest" } } } }, responses: { "200": { description: "Sessions revoked and replacement session issued" }, "401": { description: "Account is inactive or authorization generation is missing, invalid, or stale", "x-patternly-error-codes": ["app_check_required", "app_check_invalid", "authentication_required", "account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "409": { description: "Authorization generation changed or another security operation owns the account slot", "x-patternly-error-codes": ["authorization_generation_conflict", "session_revocation_operation_conflict", "session_revocation_in_progress", "security_operation_conflict"] }, "503": { description: "Revocation or replacement-session issuance pending; account remains bound", "x-patternly-error-codes": ["app_check_not_configured", "firestore_not_ready", "authentication_not_configured", "session_revocation_pending", "session_reissue_failed", "session_revocation_identity_unavailable"] } } } },
    "/v1/account/deletion": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", post: { description: "Delete the authenticated account after recent reauthentication and return a verified proof", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/AccountDeletionRequest" } } } }, responses: { "200": { description: "Remote deletion and proof complete" }, "401": { description: "Account is inactive or authorization generation is missing, invalid, or stale", "x-patternly-error-codes": ["app_check_required", "app_check_invalid", "authentication_required", "account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale", "recent_reauthentication_required"] }, "409": { description: "Authorization generation changed or another account security operation owns deletion", "x-patternly-error-codes": ["authorization_generation_conflict", "security_operation_conflict", "account_deletion_conflict", "account_deletion_in_progress"] }, "503": { description: "Remote deletion pending; retry through the bound public operation status", "x-patternly-error-codes": ["remote_deletion_pending"] } } } },
    "/v1/public/deletion-proofs/{proofId}": { "x-patternly-security-profile": "app_check_only", "x-patternly-consumer-scope": "mobile", get: { security: [], description: "Read an opaque server deletion proof", parameters: [{ name: "proofId", in: "path", required: true, schema: { type: "string" } }], responses: { "200": { description: "Deletion proof" }, "404": { description: "Proof unavailable" } } } },
    "/v1/public/deletion-operations/status": { "x-patternly-security-profile": "app_check_only", "x-patternly-consumer-scope": "mobile", post: { security: [], description: "Resume an in-app deletion after its Firebase account has been removed, using the operation-bound secret", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/PublicDeletionStatus" } } } }, responses: { "200": { description: "Deletion operation status" }, "404": { description: "Operation unavailable" } } } },
    "/v1/tracks": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", get: { responses: { "200": { description: "Account track access" } } } },
    "/v1/content/versions": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", get: { responses: { "200": { description: "Current immutable content metadata" } } } },
    "/v1/content/packages/{trackId}/{nodeId}": { "x-patternly-security-profile": "app_check_bearer", "x-patternly-consumer-scope": "mobile", get: { description: "Stream current immutable gzip package bytes after a fresh RevenueCat entitlement read. Local filesystem package storage is development-only.", parameters: [{ name: "trackId", in: "path", required: true, schema: { type: "string", pattern: "^[a-z0-9][a-z0-9._-]{0,127}$" } }, { name: "nodeId", in: "path", required: true, schema: { type: "string", pattern: "^[a-z0-9][a-z0-9._-]{0,127}$" } }], responses: { "200": { description: "Exact verified gzip package bytes", headers: { "Content-Length": { required: true, schema: { type: "integer", minimum: 1 } }, "Cache-Control": { required: true, schema: { type: "string", const: "private, no-store" } }, "X-Content-Package-Sha256": { required: true, schema: { type: "string", pattern: "^[a-f0-9]{64}$" } }, "X-Content-Artifact-Sha256": { required: true, schema: { type: "string", pattern: "^[a-f0-9]{64}$" } }, "X-Content-Version": { required: true, schema: { type: "string" } }, "X-Content-Release-Id": { required: true, schema: { type: "string", minLength: 1 } }, "X-Content-Minimum-App-Version": { required: true, schema: { type: "string", pattern: "^\\d+\\.\\d+\\.\\d+(?:[-+][A-Za-z0-9.-]+)?$" } }, "X-Content-Artifact-Size-Bytes": { required: true, schema: { type: "integer", minimum: 1 } } }, content: { "application/gzip": { schema: { type: "string", format: "binary" } } } }, "400": { description: "Invalid track or node identifier" }, "401": { description: "Authentication or App Check required" }, "403": { description: "Current entitlement is required" }, "404": { description: "Unpublished package" }, "503": { description: "Entitlement reader or package bytes unavailable" } } } },
    "/v1/content/reports": { "x-patternly-security-profile": "app_check_optional_bearer", "x-patternly-consumer-scope": "mobile", post: { security: [{ appCheckAuth: [] }], description: "App Check is required. Account and contact linkage are stored only when explicitly requested; account linkage additionally requires an optional Firebase bearer token.", parameters: [{ "$ref": "#/components/parameters/FirebaseAppCheck" }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/CreateContentReport" } } } }, responses: { "201": { description: "Content report accepted" }, "200": { description: "Duplicate client submission" }, "400": { description: "Invalid report" }, "401": { description: "Valid App Check required, account unavailable, or account linkage authentication required", "x-patternly-error-codes": ["app_check_required", "app_check_invalid", "authentication_required", "account_deleted", "authorization_generation_required", "authorization_generation_invalid", "authorization_generation_stale"] }, "409": { description: "Account authorization changed during report creation", "x-patternly-error-codes": ["authorization_generation_conflict"] }, "429": { description: "Anonymous report rate limit exceeded" } } } },
  ...
  components: {
    ...
    schemas: {
      ...
      SyncRequest: {
        type: "object",
        additionalProperties: false,
        required: ["canonicalVersion", "expectedAccountRevision", "deviceId", "sessionId", "batchId", "highWatermark", "mutations"],
        properties: {
      ...
      AdoptionConfirmationRequest: {
        ...
        properties: {
          ...
          confirmation: {
            type: "object",
            additionalProperties: false,
            required: ["operationId", "previewFingerprint", "resolutions", "groupChoices"],
            properties: {
              operationId: { type: "string", format: "uuid" },
              previewFingerprint: { type: "string", pattern: "^[a-f0-9]{64}$" },
              resolutions: adoptionTransferResolutionItems,
      ...
      AdoptionTransferStart: adoptionTransferStartOpenApi,
      ...
      AdoptionTransferUpload: adoptionTransferUploadOpenApi,
      AdoptionTransferSeal: adoptionTransferSealOpenApi,
      AdoptionTransferPreview: adoptionTransferPreviewOpenApi,
      AdoptionTransferConfirm: adoptionTransferConfirmOpenApi,
      AdoptionTransferApply: adoptionTransferApplyOpenApi,
      AdoptionTransferStatus: adoptionTransferStatusOpenApi,
      ...
      CreateGuestPrivacyRequest: { type: "object", additionalProperties: false, required: ["clientRequestId", "email", "right"], properties: { clientRequestId: { type: "string", format: "uuid" }, email: { type: "string", format: "email", maxLength: 320 }, right: { "$ref": "#/components/schemas/PrivacyRight" }, narrative: { type: "string", minLength: 1, maxLength: 2000 }, reportSubmissionIds: { type: "array", maxItems: 10, items: { type: "string", format: "uuid" } } } },
      GuestPrivacyResend: { type: "object", additionalProperties: false, required: ["email"], properties: { email: { type: "string", format: "email", maxLength: 320 } } },
      GuestPrivacyCode: { type: "object", additionalProperties: false, required: ["code"], properties: { code: { type: "string", pattern: "^pr_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\\.[A-Za-z0-9_-]{43}$" } } },
...
const progressRecordSchema = objectSchema(progressRecordProperties, ["kind", "recordType", "trackId", "targetId", "version", "fingerprint", "state", "lastMutationId", "updatedAt"]);
...
const guestMergeRecordSchema = objectSchema(guestMergeRecordProperties, ["fingerprint", "recordId", "recordType", "state", "trackId", "version"]);
...
const adoptionPreviewProperties = {
  ...
  goalPlanConflictGroups: { type: "array", maxItems: 1000, items: objectSchema({
    groupId: { type: "string", pattern: "^track:.+" },
    trackId: { type: "string", minLength: 1 },
...
const adoptionPreviewSchema = objectSchema({ ...adoptionPreviewProperties }, ["accountSnapshotVersion", "accountUserId", "conflicts", "fingerprint", "guestSnapshotVersion", "guestUserId", "operationId", "goalPlanConflictGroups"]);
...
const entitlementSchema = objectSchema({
  accountId: { type: "string", minLength: 1 },
  entitlement: { type: "string", minLength: 1 },
  productId: { type: "string", minLength: 1 },
  state: { type: "string", enum: ["active", "grace", "hold", "expired", "refunded"] },
  source: { const: "revenuecat" },
  providerExpiresAt: { anyOf: [{ type: "string", format: "date-time" }, { type: "null" }] },
  providerGraceExpiresAt: { anyOf: [{ type: "string", format: "date-time" }, { type: "null" }] },
  providerObservedAt: { type: "string", format: "date-time" },
}, ["accountId", "entitlement", "productId", "state", "source", "providerExpiresAt", "providerGraceExpiresAt", "providerObservedAt"]);
...
const successSchemas: Readonly<Record<string, OpenApiRecord>> = {
  ...
  "GET /ready": objectSchema({ status: { type: "string", enum: ["ready", "not_ready"] }, checks: objectSchema({ database: { type: "boolean" }, authentication: { type: "boolean" }, providerReader: { type: "boolean" } }, ["database", "authentication", "providerReader"]) }, ["status", "checks"]),
  ...
  "POST /v1/account/session/exchange": objectSchema({ customToken: { type: "string", minLength: 1 } }, ["customToken"]),
  ...
  "GET /v1/entitlements": objectSchema({ serverObservedAt: { type: "string", format: "date-time" }, entitlements: { type: "array", minItems: 1, maxItems: 1, items: entitlementSchema } }, ["serverObservedAt", "entitlements"]),
  ...
  "POST /v1/guest/privacy-requests": objectSchema({ status: { type: "string", const: "pending_verification" }, requestId: { type: "string", pattern: "^pr_[0-9a-f-]{36}$" } }, ["status", "requestId"]),
  "POST /v1/guest/privacy-requests/verify": objectSchema({ requestId: { type: "string", pattern: "^pr_[0-9a-f-]{36}$" }, sessionToken: { type: "string", minLength: 32 } }, ["requestId", "sessionToken"]),
  "POST /v1/guest/privacy-requests/{requestId}/resend": objectSchema({ status: { type: "string", const: "pending_verification" } }, ["status"]),
  "POST /v1/guest/privacy-requests/{requestId}/response": ref("PrivacyRequestResponse"),
  ...
  "POST /v1/account-data/adoption/transfer/start": ref("AdoptionTransferStatus"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/upload": ref("AdoptionTransferStatus"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/seal": ref("AdoptionTransferStatus"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/preview": ref("AdoptionTransferPreviewResponse"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/confirm": ref("AdoptionTransferStatus"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/apply": ref("AdoptionTransferStatus"),
  "GET /v1/account-data/adoption/transfer/{sessionId}/status": ref("AdoptionTransferStatus"),
  ...
  "POST /v1/account/session/revoke": objectSchema({ status: { type: "string", const: "revoked" }, operationId: { type: "string", format: "uuid" }, customToken: { type: "string", minLength: 1 } }, ["status", "operationId", "customToken"]),
  ...
  "GET /v1/content/packages/{trackId}/{nodeId}": { type: "string", format: "binary" },
...
const requestSchemas: Readonly<Record<string, OpenApiRecord>> = {
  ...
  "POST /v1/account/session/exchange": ref("EmptyRequest"),
  ...
  "POST /v1/guest/privacy-requests": ref("CreateGuestPrivacyRequest"),
  "POST /v1/guest/privacy-requests/verify": ref("GuestPrivacyCode"),
  "POST /v1/guest/privacy-requests/{requestId}/resend": ref("GuestPrivacyResend"),
  "POST /v1/guest/privacy-requests/{requestId}/response": ref("PublicPrivacySession"),
  ...
  "POST /v1/account-data/adoption/transfer/start": ref("AdoptionTransferStart"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/upload": ref("AdoptionTransferUpload"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/seal": ref("AdoptionTransferSeal"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/preview": ref("AdoptionTransferPreview"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/confirm": ref("AdoptionTransferConfirm"),
  "POST /v1/account-data/adoption/transfer/{sessionId}/apply": ref("AdoptionTransferApply"),
...
function enrichOpenApiDocument(document: OpenApiRecord): void {
  ...
  schemas.AdoptionTransferPreviewResponse = objectSchema(adoptionTransferPreviewResponseProperties, adoptionTransferPreviewResponseRequired);
  ...
  for (const [path, pathItemValue] of Object.entries(paths)) {
    ...
    for (const [method, operationValue] of Object.entries(pathItem)) {
      ...
      operation.security = securityNames.length === 0 ? [] : [Object.fromEntries(securityNames.map((name) => [name, []]))];
      ...
      if (["POST", "PUT", "PATCH", "DELETE"].includes(method.toUpperCase())) {
        ...
        const requestBodyRequired = (operation.requestBody as OpenApiRecord | undefined)?.required !== false;
        operation.requestBody = { required: requestBodyRequired, content: { "application/json": { schema: requestSchema } } };
      }
      const responses = (operation.responses as OpenApiRecord | undefined) ?? {};
      operation.responses = responses;
      if (securityProfile === "bearer" || securityProfile === "app_check_bearer" || securityProfile === "app_check_optional_bearer") {
      ...
      for (const [status, responseValue] of Object.entries(responses)) {
        ...
        if (numericStatus >= 400) {
          response.content = { "application/json": { schema: identity === "GET /ready" && numericStatus === 503
            ? successSchema
