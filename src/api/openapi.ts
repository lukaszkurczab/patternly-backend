export const OPENAPI_DOCUMENT = Object.freeze({
  ...
  paths: {
    ...
    "/v1/admin/content-reports": { get: { description: "Open and in-review content reports for the configured administrator", responses: { "200": { description: "Content report triage queue" }, "403": { description: "Administrator required" } } } },
    "/v1/admin/content-reports/{clientSubmissionId}": { patch: { description: "Advance one report through the server-owned triage state machine; Firebase administrator authentication is required.", parameters: [{ name: "clientSubmissionId", in: "path", required: true, schema: { type: "string", format: "uuid" } }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/TransitionContentReport" } } } }, responses: { "200": { description: "Status transition applied or repeated idempotently" }, "400": { description: "Invalid status transition request" }, "403": { description: "Administrator required" }, "404": { description: "Report not found" }, "409": { description: "Status transition is not allowed" } } } },
  },
  components: {
