...
const OPENAPI_DOCUMENT_RAW = {
  ...
  paths: {
    ...
    "/v1/guest/privacy-requests": { "x-patternly-security-profile": "app_check_only", "x-patternly-consumer-scope": "mobile", post: { description: "Record an idempotent guest privacy request and start email possession verification in the app", requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/CreateGuestPrivacyRequest" } } } }, responses: { "202": { description: "Request pending verification" }, "401": { description: "App Check required" }, "409": { description: "Idempotency key reused with different payload" }, "429": { description: "Rate limited" }, "503": { description: "Email sender unavailable" } } } },
    ...
    "/v1/guest/privacy-requests/{requestId}/resend": { "x-patternly-security-profile": "app_check_only", "x-patternly-consumer-scope": "mobile", post: { description: "Non-enumerating request for a fresh in-app email code", parameters: [{ "$ref": "#/components/parameters/PrivacyRequestId" }], requestBody: { required: true, content: { "application/json": { schema: { "$ref": "#/components/schemas/GuestPrivacyResend" } } } }, responses: { "202": { description: "Verification pending" }, "429": { description: "Rate limited" }, "503": { description: "Email sender unavailable" } } } },
