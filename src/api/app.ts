...
export function buildApplication(dependencies: ApplicationDependencies) {
  ...
  app.post("/v1/account-data/adoption/transfer/:sessionId/preview", { preHandler: routeGuard("bearer", dependencies) }, async (request, reply) => {
    ...
    if (!sessionId || !parsed.success) return reply.code(400).send({ error: { code: "invalid_request", ...(parsed.success ? {} : { issues: parsed.error.issues.map((issue) => issue.path.join(".")) }) } });
    try {
      return reply.code(200).send(await requireStores(dependencies).progress.previewAdoptionTransfer(request.userId!, request.expectedAuthorizationGeneration!, sessionId, parsed.data));
    } catch (error) {
      if (adoptionTransferErrorResponse(error, reply)) return;
      throw error;
    }
  ...
  app.post("/v1/account-data/adoption/transfer/:sessionId/apply", { preHandler: routeGuard("bearer", dependencies) }, async (request, reply) => {
    ...
    if (!sessionId || !parsed.success) return reply.code(400).send({ error: { code: "invalid_request", ...(parsed.success ? {} : { issues: parsed.error.issues.map((issue) => issue.path.join(".")) }) } });
    try {
      return reply.code(200).send(await requireStores(dependencies).progress.applyAdoptionTransfer(request.userId!, request.expectedAuthorizationGeneration!, sessionId, parsed.data));
    } catch (error) {
      if (adoptionTransferErrorResponse(error, reply)) return;
      throw error;
    }
